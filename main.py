...
# Hero Page
def hero_page():
    st.markdown('<h1 class="hero-title">AI Learning Companion</h1>', unsafe_allow_html=True)
    st.markdown('<p class="hero-subtitle">Personalized Multi-Agent Tutoring Experience</p>', unsafe_allow_html=True)
...
def screener_page():
    st.markdown('<h1 style="color: #667eea;">Let\'s Get to Know You</h1>', unsafe_allow_html=True)
    st.markdown('<p style="color: #a0a0c0;">Help us personalize your learning experience</p>', unsafe_allow_html=True)
    ...
    with chat_container:
        ...
        if st.session_state.screener_step == 0:
            with st.chat_message("assistant"):
                st.write("Hello! I'm your AI Learning Companion. Let's start by getting to know you better.")
            ...
            with st.chat_message("assistant"):
                ...
                st.write(f"- **Grade Level:** {st.session_state.student_profile.get('grade', 'N/A')}")
                st.write(f"- **Interest Area:** {st.session_state.student_profile.get('interest', 'N/A')}")
                st.write(f"- **Learning Pace:** {st.session_state.student_profile.get('pace', 'N/A')}")
...
# Learning Page
def learning_page():
    st.markdown('<h1 style="color: #667eea;">Learning Hub</h1>', unsafe_allow_html=True)
    ...
    with chat_container:
        if st.session_state.learning_topic:
            with st.chat_message("assistant"):
                st.write(f"Great! Let's learn about **{st.session_state.learning_topic}**.")
        ...
        with chat_container:
            ...
            with st.chat_message("assistant"):
                ...
                with st.spinner("Thinking..."):
                    time.sleep(1)
                st.write(f"Here's a detailed explanation about your question regarding {st.session_state.learning_topic}...")
...
# Quiz Page
def quiz_page():
    st.markdown('<h1 style="color: #667eea;">Knowledge Assessment</h1>', unsafe_allow_html=True)
    
    if st.session_state.learning_topic:
        st.markdown(f'<p style="color: #a0a0c0;">Testing your understanding of: <b>{st.session_state.learning_topic}</b></p>', unsafe_allow_html=True)
        ...
        if submitted:
            ...
            for i, q in enumerate(st.session_state.quiz_questions):
                if q["type"] == "mcq":
                    ...
                    user_answer = st.session_state.quiz_answers.get(i) == "True"
            ...
            if score_percentage >= 70:
                ...
                st.warning(f"📚 Good effort! You scored {score_percentage:.0f}%. Keep practicing!")
            else:
                st.error(f"💪 You scored {score_percentage:.0f}%. Let's review the material again.")
            ...
            for i, q in enumerate(st.session_state.quiz_questions):
                with st.expander(f"Question {i+1}"):
                    st.write(q["question"])
                    if q["type"] == "short":
                        st.write(f"**Your answer:** {st.session_state.quiz_answers.get(i, 'No answer')}")
...
def sidebar_navigation():
    with st.sidebar:
        st.markdown('<h2 style="color: #667eea;text-align: center;">Navigation</h2>', unsafe_allow_html=True)
        ...
        if st.session_state.student_profile:
            st.markdown('<h3 style="color: #764ba2;">Your Profile</h3>', unsafe_allow_html=True)
            st.write(f"**Grade:** {st.session_state.student_profile.get('grade', 'Not set')}")
            st.write(f"**Interest:** {st.session_state.student_profile.get('interest', 'Not set')}")
            st.write(f"**Pace:** {st.session_state.student_profile.get('pace', 'Not set')}")
        
        # Progress metrics
        if st.session_state.quiz_score > 0:
            st.markdown("---")
            st.markdown('<h3 style="color: #764ba2;">Progress</h3>', unsafe_allow_html=True)
